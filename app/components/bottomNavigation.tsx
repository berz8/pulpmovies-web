...
const menuItems: MenuItem[] = [
  ...
  { id: 1, label: "Watchlist", path: "/lists", matchingPaths: ["/lists"] },
  {
    ...
    label: "Search",
    path: "/search",
    matchingPaths: ["/search", "/movie", "/person"],
  },
  {
    id: 3,
    label: "Profile",
    path: "/profile",
    matchingPaths: ["/profile", "/login"],
  },
];
...
export default function BottomNavigation() {
  ...
  const activeId = useCallback(
    (menuItems: MenuItem[]): number => {
      let x = 0;
      menuItems.forEach((menuItem) => {
        if (
          menuItem.matchingPaths.find((x) => x === "/") &&
          location.pathname !== "/"
        ) {
          x = menuItem.id;
        } else if (
          menuItem.matchingPaths.find((x) => location.pathname.includes(x))
        ) {
          x = menuItem.id;
        }
      });
        ...
        <div className="relative flex w-full justify-around gap-x-1 lg:gap-y-4">
          <motion.div
            ...
            style={{
              ...
              left: `${(100 / menuItems.length) * activeId(menuItems)}%`,
            }}
          />
