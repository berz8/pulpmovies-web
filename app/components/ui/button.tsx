export function Button({ text, type = "button", disabled = false, variant = "main", size = "md", icon, onClick }: Props) {
  return (
    <button
      type={type}
...
const classGenerator = (variant: string, size: string): string => {
  let className = "w-full flex gap-2 justify-center rounded-md disabled:bg-none disabled:bg-[rgba(0,0,0,0.3)] disabled:shadow-inner disabled:text-[rgba(255,255,255,0.6)] ";
  if (variant === "main") className += " bg-gradient-to-br from-[#E200B1] to-[#6400E2] text-white ";
  if (variant === "secondary") className += "bg-[rgba(0,0,0,0.3)] shadow-md text-gray-200 ";
