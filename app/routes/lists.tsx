...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const wathclistsReq = await fetch(
    `${process.env.API_URL}/watchlist/user/${user.user.id}`,
    {
      headers: {
  ...
  let watchlistWithMovies: { watchlist: Watchlist; movies: Movie[] } | null =
    null;
  if (watchlists.length > 0) {
    const watchlistWithMoviesReq = await fetch(
      `${process.env.API_URL}/watchlist/id/${watchlists[0].id}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.token}`,
...
export default function ListsPage() {
  ...
  const posterPath = (movie: Movie) =>
    !movie.poster_path
      ? "/images/fallback-movie.jpg"
      : `https://image.tmdb.org/t/p/w200${movie.poster_path}`;
  return (
    <div className="pt-4 px-3">
      <h2 className="text-2xl text-gray-400 font-bold">Your lists</h2>
      <div className="mt-3">
      ...
      <div className="mt-4">
        {watchlists.map((w) => (
          <Link
            to={`/lists/${w.id}`}
            key={w.id}
            className="mb-4 block w-full rounded-lg shadow-lg bg-[rgba(0,0,0,0.3)] py-2 px-2"
          >
            <div className="flex items-center justify-between text-gray-300 ">
              ...
              <div className="flex gap-1">
                <span>
                  {watchlistWithMovies
                    ? watchlistWithMovies.movies.length
                    : "0"}{" "}
                  movie{watchlistWithMovies?.movies.length != 1 ? "s" : null}
                </span>
            ...
            {watchlistWithMovies && watchlistWithMovies.movies.length > 0 ? (
              <div className="flex gap-2 mt-2">
                {watchlistWithMovies &&
                  watchlistWithMovies.movies.slice(0, 6).map((m) => (
              ...
              <div className="py-8 px-4 text-center">
                <span className="italic text-gray-400">
                  You can add movies to this list directly from the movie page
                </span>
              </div>
            )}
          </Link>
