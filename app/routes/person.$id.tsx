...
export default function PersonId() {
  ...
  const creditsCategories = () => {
    ...
    if (person.movie_credits.cast && person.movie_credits.cast.length > 0)
      res.push({ id: 0, value: "acting", label: "Actor", detail: person.movie_credits.cast.length.toString() })
    if (directorCount > 0)
      res.push({ id: 1, value: "directing", label: "Director", detail: directorCount.toString() })
    if (crewCount > 0)
      res.push({ id: 2, value: "crew", label: "Crew", detail: crewCount.toString() })
  ...
  const defaultSelectedcategory = () => {
    ...
    return { value: knownFor, index: creditsCategories().findIndex(x => x.value === knownFor)};
  }
