...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (query && query !== "") {
    ...
    if (section === searchResultsType.person)
      result.person = resJson.results as PersonSearch[];
  }
  return json(result);
};
...
export default function Index() {
  ...
  const selectedSection = () => {
    const index = Object.keys(searchResultsType).findIndex(
      (x) => x === section,
    );
  ...
  return (
    <div className="p-4">
      <div
        id="search"
        className="fixed z-30 top-0 left-0 w-full p-4 pb-0 bg-transparent backdrop-blur-md"
      >
        <div
        ...
        >
          ...
          <div className="mt-3 p-1 rounded-lg bg-[rgba(0,0,0,0.3)] shadow-md w-full md:w-1/2 lg:mt-0">
            <SegmentedControls
              ...
              setSelected={handleSectionChange}
            />
          </div>
        </div>
      </div>
      ...
      {!!(person.length && section === searchResultsType.person) && (
        <div className="pt-32 pb-14 m-auto md:pt-20 flex flex-col lg:flex-wrap lg:gap-4 lg:flex-row">
          {person.map((pers) => (
            <PersonSearchCard key={pers.id} person={pers} />
          ))}
        </div>
      )}
    </div>
